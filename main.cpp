...
{
	map<string, Model> planets;
	Model Sun( (modelsPath + "Sun").c_str() );
	planets.insert(pair<string, Model>("Sun", Sun));
	Model Earth((modelsPath + "Earth").c_str());
	planets.insert(pair<string, Model>("Earth", Earth));
	return planets;
	
}
void RenderModels(Shader& modelShader,map<string,Model>&planets)
{
